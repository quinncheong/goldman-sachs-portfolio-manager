    ...
    public Map<String, PortfolioCalculator> getCalculatedStockInPortfolio(ObjectId id) {
        ...
        for (AllocatedStock allocatedStock : allocatedStocks) {
            ...
            if (!calculatedStock.containsKey(stockTicker)) {
                ...
                PortfolioCalculator portfolioCalculator = new PortfolioCalculator(stockTicker, quantity,
                        quantity * allocatedStock.getStockBuyPrice(), 0.0, 0.0);
                ...
                portfolioCalculator.setAvgStockBuyPrice(
        ...
        for (String stockTicker : calculatedStock.keySet()) {
            ...
            // double value = portfolioCalculator.getStockQuantity() *
            // stock.getPriceToday();
            double value = portfolioCalculator.getStockQuantity() * stockService.getPriceTodayByTicker(stockTicker);
            portfolioCalculator.setAvgStockBuyPrice(
                    portfolioCalculator.getAvgStockBuyPrice() / portfolioCalculator.getStockQuantity());
